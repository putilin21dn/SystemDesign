...
EXERCISE_NAME="Squat_${SUFFIX}"
CODE=$(http_code "$BASE_URL/exercises" \
  -X POST \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"name\":\"$EXERCISE_NAME\",\"description\":\"Leg exercise\",\"muscle_group\":\"legs\"}")
echo "HTTP $CODE"
cat /tmp/fitness_resp.json | pretty
EXERCISE_ID=$(extract_json_field id)
if [[ "$CODE" != "201" || -z "$EXERCISE_ID" ]]; then
  echo "Failed to create exercise, stopping tests"
  exit 1
fi
...
if [[ "$CODE" != "201" || -z "$WORKOUT_ID" ]]; then
  echo "Failed to create workout, stopping tests"
  exit 1
fi
